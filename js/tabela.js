...
var targetBreakpoint = 900;
var currentVisibleColumn = 1;
var nextButtonText = 'See other plans';

// SETUP/SELECT REUSABLE ELEMENTS
var table = document.querySelector(tableSelector);
var allCells = table.querySelectorAll('th, td');
var columnHeaders = table.querySelectorAll('thead th:not(.preco-tabela):nth-of-type(n+1)');
...
function showCurrentlyVisible() {
  ...
  var currentlyVisibleColHeader = document.querySelector('thead th:not(:empty):nth-of-type( ' + currentVisibleColumn + ')') ;
   var currentlyVisibleColHeaderPrice = document.querySelector('.preco-tabela:nth-of-type( ' + currentVisibleColumn + ')');
...
currentlyVisibleColHeaderPrice.style.display = 'block';
...
function updateTable() {
  ...
  if (tableWidth > window.innerWidth || tableWidth < targetBreakpoint) {
    ...
    if (table.getAttribute('data-comparing') !== 'active') {
	  ...
	  nextButton.style.marginTop = '3vh';
	  nextButton.className = 'ui huge inverted secondary button seeotherplans';
